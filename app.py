...
list_job2 = ["(なし)", "騎士", "魔導騎士", "魔導士", "魔導術師", "魔導狙撃士", "錬金術師", "治癒神官", "死霊術師", "竜騎士", "狂戦士", "銃士", "武士"]
...
cursed_races = ["呪い人", "羅刹", "アルラウネ", "スチームブッチャー", "怪異憑依者／怪人"]
...
def calculate(p):
    origin = p['origin']
    level = p['level']
    lvl_num = int(level.replace("Lv", ""))
    max_sp = dict_max_sp.get(level, 0)
    spent_sp = sum(p['stats_sp'].values())
    stats = {k: p['stats_base'][k] + p['stats_sp'][k] for k in base_stats_names}
    mod_hp, mod_mp, mod_stamina, bonus_sp, mod_luck = 0, 0, 0, 0, 0
    bonus_mental = 0
    bonus_ab_melee, bonus_ab_magic = 0, 0
    warning_errors = []
    ...
    if is_cursed:
        actual_attr = "無属性"
        warning_errors.append("💡【属性固定】呪族のため、属性は「無属性」に固定されます。")
    elif is_demon:
        actual_attr = "血属性"
        warning_errors.append("💡【属性固定】魔族のため、属性は「血属性」に固定されます。")
    elif origin == "ノクターン" and race != "幻影体発現者":
        actual_attr = "無属性"
        warning_errors.append("💡【属性固定】ノクターンの非魔族のため、属性は「無属性」に固定されます。")
    ...
    sub_sp_text = f"サブSP消費: {spent_sub_sp} / {max_sub_sp}"
    if spent_sub_sp > max_sub_sp:
        warning_errors.append(f"⚠️【サブSP超過】割り振り({spent_sub_sp})が上限({max_sub_sp})を超えています。")
    sub_melee_ab_bonus = min(5, int(sub_stats['膂力'] * 0.25))
    sub_str_mod = int(sub_stats['膂力'] * 0.5)
    sub_magic_ab_bonus = min(5, int(sub_stats['叡智'] * 0.25))
    sub_int_mod = int(sub_stats['叡智'] * 0.5)
    sub_hp_bonus = min(10, int(sub_stats['体力'] * 0.5))
    mod_hp += sub_hp_bonus
    sub_stamina_bonus = min(5, int(sub_stats['持久力'] * 0.25))
    mod_stamina += sub_stamina_bonus
    sub_skill_mod = min(5, int(sub_stats['技量'] * 0.25))
    sub_eva_mod = min(5, int(sub_stats['技量'] * 0.25))
    sub_holy_mod = min(100, int(sub_stats['神聖'] * 5))
    sub_biz_mod = min(10, int(sub_stats['商売'] * 0.5))
    sub_art_mod = min(10, int(sub_stats['表現力'] * 0.5))
    sub_exp_mod = int(sub_stats['探求心'] * 0.5)
    bonus_ab_melee += sub_melee_ab_bonus
    bonus_ab_magic += sub_magic_ab_bonus
    sub_texts = []
    if sub_stats['膂力'] > 0: sub_texts.append(f"・膂力: 白兵AB+{sub_melee_ab_bonus} / 筋力判定基準値-{sub_str_mod}")
    if sub_stats['叡智'] > 0: sub_texts.append(f"・叡智: 知力AB+{sub_magic_ab_bonus} / 知力判定基準値-{sub_int_mod}")
    if sub_stats['体力'] > 0: sub_texts.append(f"・体力: HP+{sub_hp_bonus}")
    if sub_stats['持久力'] > 0: sub_texts.append(f"・持久力: スタミナ+{sub_stamina_bonus}")
    if sub_stats['技量'] > 0: sub_texts.append(f"・技量: 技能ロール+{sub_skill_mod} / 回避+{sub_eva_mod}(上限50)")
    if sub_stats['神聖'] > 0: sub_texts.append(f"・神聖: 信仰系攻撃威力+{sub_holy_mod}%")
    if sub_stats['商売'] > 0: sub_texts.append(f"・商売: 商才ロール補正+{sub_biz_mod}")
    if sub_stats['表現力'] > 0: sub_texts.append(f"・表現力: 芸術力ロール補正+{sub_art_mod}")
    if sub_stats['探求心'] > 0: sub_texts.append(f"・探求心: 探索系基準値-{sub_exp_mod}")
    combat_sub_names = ["膂力", "叡智", "体力", "持久力", "技量"]
    sub_to_main_combat = {"膂力": "筋力", "叡智": "知力", "体力": "生命", "持久力": "体格", "技量": "敏捷"}
    combat_subs = {k: sub_stats[k] for k in combat_sub_names}
    ...
    if blessing in ["地影神の加護", "風影神の加護"]: final_stamina = max(1, final_stamina)
...
# ===================================================
st.set_page_config(
    page_title="ステラリアクロニクル サポートツール",
    layout="wide",
    page_icon="⚔️"
)

st.title("⚔️ ステラリアクロニクル サポートツール")
...
with st.sidebar:
    ...
    st.markdown("---")
    st.caption("計算後、下のボタンでデータをJSON保存できます。")
...
with col_left:
    ...
    with c2: age = st.selectbox("代（年代）", list_age, index=si('age', list_age), key='age')
...
with col_right:
    ...
    if 'last_result' in st.session_state:
        ...
        simple_keys = ['origin', 'level', 'age', 'gender', 'ethnicity', 'race', 'race_sub',
                       ...
                       'job1_1', 'job1_1_lv', 'job1_2', 'job1_2_lv',
