...
list_race_fantasia = list_race_common + [
    ...
    "フレイムエルフ", "ウーンゲフォイヤー", "ヴァンシー", "クラーケンF","マンティコアF", "メルフェディオヌF", "ローレライ", "モーデュリア"
]
...
list_bg_fantasia = ["騎士", "賊", "魔術師", "聖職者", "貴族", "放浪戦士", "芸術家", "盗人", "狩人", "侍", "窶し者", "双剣士", "銃剣士", "商人", "淑女", "通常使用人", "戦闘使用人", "多重人格者", "対偶者", "海賊", "祈祷者", "蛮族", "贖血徒", "迷い人"]
...
list_attr = ["無属性", "血属性", "地属性", "花属性", "雪属性", "氷属性", "水属性", "風属性", "炎属性", "雷属性", "光属性", "夜属性", "鉱属性", "空属性", "暗属性", "双属性"]
list_attr_sub = ["地属性", "花属性", "雪属性", "氷属性", "水属性", "風属性", "炎属性", "雷属性", "光属性", "夜属性", "鉱属性", "空属性", "暗属性"]
...
def calculate():
    ...
    lvl_bonus_sp_stages = {
        0: 0,
        1: 20 if origin == "ノクターン" else 40,
        2: 30 if origin == "ノクターン" else 50,
        3: 50,
        4: 70,
        5: 80,
        6: 50,
    ...
    for a in current_attrs:
        if a == "炎属性":
            ...
            attr_sys_texts.append("【鉱属性】《百錬》他属性に対して被ダメージ-10%/与ダメージ+10%")
        elif a == "空属性":
            attr_sys_texts.append("【空属性】《霄漢》光&夜を除く全ての属性からの被ダメージ-10%/炎&風&雷に対し、与ダメージ+20%")
        elif a == "暗属性":
            attr_sys_texts.append("【暗属性】《幽暗》光&夜を除く全ての属性に対して回避基準値+10/雪&水&地に対し、与ダメージ+20%")
    ...
    if origin == "ファンタジア" or (origin == "ノクターン" and (race in list_race_common or race in list_race_nocturne)):
        if race == "人間族":
                ...
                eva_limit_break += 2
                bonus_sp += 150
                if race_sub == "ベスティアケット": stats["容姿"] -= 5; mod_hp += 7; mod_credit -= 10
                elif race_sub == "アニマケット": stats["容姿"] += 2; mod_mp += 2; mod_credit -= 2
        elif race == "妖怪":
                mod_credit -= 3
...
職業戦技『武士道』【常時発動/剣戟使用時、居合威力+50%】""")
    ...
    val_extra_credit = p.get('extra_credit', 0)
    ...
    final_credit = base_credit + mod_credit
    if is_blessing_active:
...
with col_right:
    ...
    if 'last_result' in st.session_state:
        ...
        simple_keys = ['origin', 'level', 'age', 'gender', 'ethnicity', 'race', 'race_sub',
                       ...
                       'extra_hp', 'extra_mp', 'extra_stamina', 'extra_evasion', 'extra_text', 'extra_credit',
